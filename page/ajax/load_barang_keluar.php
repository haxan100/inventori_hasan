...
// memanggil file config.php
    ...
    $from = '`tb_barang_keluar` tbm inner join tb_barang tb on tbm. kode_barang=tb.kode_barang inner join tb_satuan ts on tb.satuan=ts.id_satuan INNER join tb_rak tr on tb.no_rak=tr.no_rak';
...
// die;
    ...
    while ($row = $data->fetch_array()) {
        ...
        // var_dump($fields);die;
        $fields[] = $row['kode_barang_keluar'];
        $fields[] = $row['nama_barang'];
        $fields[] = $row['jenis_barang'];
        $fields[] = $row['nama_rak'];
        $fields[] = $row['tanggal_keluar'];
        $fields[] = $row['nama_satuan'];
        $fields[] = $row['jumlah_keluar'];
    ...
    // echo $sql;
        ...
        data-kode_barang_keluar="'.$row['kode_barang_keluar']. '" data-jenis_barang="' . $row['jenis_barang'] . '" data-jumlah_keluar="' . $row['jumlah_keluar'] . '" 
        ...
        <button href="#" class="btn btn-danger btn-sm mb-1 tombolHapus" data-nama_barang="'.$row['nama_barang'].'" data-kode_barang_keluar="'.$row['kode_barang_keluar']. '" ">
